...
ds_name = "image-paragraph-captioning"  # change the dataset name here
dataset = load_dataset("MMInstruction/M3IT", ds_name)
split = 'validation'
train_set = dataset[split]  
dataroot = 'playground/data'
image_folder = 'image-paragraph-captioning/images/%s' % split
...
for id, train_instance in enumerate(tqdm(train_set)):
    ...
    conv = []
    conv.append({'from': 'human', 'value': '<image>\n' + instruction + train_instance['inputs']})
    conv.append({'from': 'gpt', 'value': train_instance['outputs']})
    item['conversations'] = conv
    item['image_size'] = image_0.size
    new_data.append(item)

new_json_file = 'image-paragraph-captioning_%s_process.json' % split
if not os.path.exists('playground/data/image-paragraph-captioning'):
    os.makedirs('playground/data/image-paragraph-captioning')
dst_json_file = os.path.join('playground/data/image-paragraph-captioning', new_json_file)
print(len(new_data))
