...
for da in tqdm(data):
    ...
    img_path = os.path.join(dataroot, image_folder, '%05d.jpg' % count)
    da["image"].save(img_path)

    json_data["id"] = os.path.join(image_folder, '%05d.jpg' % count)
    json_data["image"] = os.path.join(image_folder,'%05d.jpg' % count)
...
new_json_file = 'finqa_%s_process.json' % split
if not os.path.exists('playground/data/sft_oct24/finqa'):
    os.makedirs('playground/data/sft_oct24/finqa')
dst_json_file = os.path.join('playground/data/sft_oct24/finqa', new_json_file)
print(len(new_data))
