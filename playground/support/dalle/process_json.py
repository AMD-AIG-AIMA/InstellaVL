...
def process_image_data(root_path, image_path, start_value=0, end_value=500):
    ...
    dirs = ['data-%06d' % i for i in range(start_value, end_value)]
    ...
    for folder in tqdm(dirs):
        ...
        for json_file in json_files:
            ...
            item = {}
            item['id'] =  os.path.join(image_path, folder, data['image_name'])
            item['image'] = os.path.join(image_path, folder, data['image_name'])
    ...
    json_file = f'packed_jsons/images_{start_value:05d}_{end_value:05d}.json'
