...
class CustomTempDirectory:
    def __init__(self, base_dir: str, prefix: str = 'tmp_'):
        """
...
def create_temp_dir(base_path: str = os.path.join(os.environ['CACHE_DIR'], 'checkpoints'), prefix: str = 'tmp-model-') -> CustomTempDirectory:
    """
...
class OCIFolderCheckpointHandler:
    ...
    def save_checkpoint(
        ...
        self._upload_directory_recursively(temp_dir.name, checkpoint_folder)
...
if __name__ == "__main__":
    ...
    bucket_name = 'ocieobject1'
    region_name = "us-ashburn-1"

    handler = OCIFolderCheckpointHandler(access_key_id, secret_access_key, endpoint_url, bucket_name, region_name)
    print(handler.checkpoint_exists('ximensun/checkpoints/002-checkpoints/'))
    temp_dir = handler.load_checkpoint('ximensun/checkpoints/002-checkpoints/llavanext-google_siglip-so400m-patch14-384-AIG-GenAI_AMD-OLMo-1B-SFT-mlp2x_gelu-pretrain_blip558k_plain-full-finetune-stage1.5/', CustomTempDirectory(base_dir='aaa'))
    handler.save_checkpoint(temp_dir,'ximensun/checkpoints/002-checkpoints/llavanext-google_siglip-so400m-patch14-384-AIG-GenAI_AMD-OLMo-1B-SFT-mlp2x_gelu-pretrain_blip558k_plain-full-finetune-stage1.5-3' )
    import pdb
    pdb.set_trace()